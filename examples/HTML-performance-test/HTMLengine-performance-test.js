...
HTML.render(document.body, test)
	//.datepicker(test.CurrentDate).$()
	//.datepicker(test.CurrentDate).$().br()
...
HTML.render(document.body, test)
	.div().classes(test.divClss).attr({title: 'This is my title'})
		.each(test.children, function(model, index){
            HTML.render(this)
                .span(index).$()
                .checkbox(model.checked).change(test.checkChange).$()
                .span(model.DisplayName).$()
                .input(model.Name).change(model.f5).$()
                .input(model.Age).change(model.f5).$()
                .span('Render at: ').$().span(model.timeFormat).$()
				.button('Delete').click(test.DeletePerson, model).$()
				.br();
        })
    .$()
    ...
    .button('3.000 children').click(test.add3000).$()
	.br().span('Large data').$().br()
    .button('6.000 children').click(test.add6000).$()
	.button('10.000 children').click(test.add10000).$()
    .button('100.000 children').click(test.add100000).$()
    //.div()
	//	.br().br().br().br()
	//	.span('FullName: ').$()
	//	.span(test.FullName).$().br()
    //    .span('Title: ').$()
	//	.span(test.txtTitle).$().br()
	//	.input(test.txtLastName).$()
	//	.input(test.txtFirstName).$()
    //    .input(test.txtLastName).$()
	//	.input(test.txtFirstName).$()
	//	.input(test.txtTitle).$()
	//.$()
	...
	//.span(test.txtLastName).$().span(test.txtFirstName).$().br()
    //.span(test.txtLastName).$().span(test.txtFirstName).$().br()
	//.span(test.FullName).$().br()
	//.span(test.FullName).$().br()
	//.span(test.FullName).$().br()
//txtLastName('Oh yeah, I did it');